...
type SafeUser = Omit<UserAttributes, 'password' | 'reset_password_token' | 'reset_password_expires'>;
...
export class AuthService {
  ...
  async register(data: RegisterDto): Promise<{ token: string; user: SafeUser }> {
    ...
    const user = await User.create({ ...data, password: hashed, role: UserRole.CLIENT });
